...
INSERT INTO tournaments (id, name, logo, description, category, level, location, start_date, end_date, status, format, max_pairs, registered_pair_ids, rules, court_ids) VALUES ('tour_major_madrid', 'Premier Padel Major Madrid 2026', '🏆', 'El torneo oficial más prestigioso de la temporada con las mejores 16 parejas del circuito.', 'Masculino', 'Profesional', 'Wizink Center, Madrid', '2026-08-10', '2026-08-16', 'ACTIVE', 'Grupos + eliminación directa', '16', '["pair_galan_lebron","pair_coello_tapia","pair_chingotto_navarro","pair_stupaczuk_dinenno"]', '{"sets_to_win":2,"golden_point":true,"tie_break_at":6,"final_set_tie_break":true,"points_distribution":{"champion":1000,"runner_up":600,"semi_finals":360,"quarter_finals":180,"group_stage":90}}', '["crt_central","crt_2"]') ON DUPLICATE KEY UPDATE id = VALUES(id), name = VALUES(name), logo = VALUES(logo), description = VALUES(description), category = VALUES(category), level = VALUES(level), location = VALUES(location), start_date = VALUES(start_date), end_date = VALUES(end_date), status = VALUES(status), format = VALUES(format), max_pairs = VALUES(max_pairs), registered_pair_ids = VALUES(registered_pair_ids), rules = VALUES(rules), court_ids = VALUES(court_ids);
INSERT INTO tournaments (id, name, logo, description, category, level, location, start_date, end_date, status, format, max_pairs, registered_pair_ids, rules, court_ids) VALUES ('tour_open_barcelona', 'Copa Abierta Barcelona 2026', '🎾', 'Competición de categoría Mixto Abierto con fases de grupos intensas y eliminatorias.', 'Mixto', 'Avanzado', 'Real Club de Tenis Barcelona', '2026-09-01', '2026-09-05', 'REGISTRATION', 'Fase de grupos', '12', '["pair_chingotto_navarro","pair_stupaczuk_dinenno"]', '{"sets_to_win":2,"golden_point":true,"tie_break_at":6,"final_set_tie_break":true,"points_distribution":{"champion":500,"runner_up":300,"semi_finals":180,"quarter_finals":90,"group_stage":45}}', '["crt_3","crt_4"]') ON DUPLICATE KEY UPDATE id = VALUES(id), name = VALUES(name), logo = VALUES(logo), description = VALUES(description), category = VALUES(category), level = VALUES(level), location = VALUES(location), start_date = VALUES(start_date), end_date = VALUES(end_date), status = VALUES(status), format = VALUES(format), max_pairs = VALUES(max_pairs), registered_pair_ids = VALUES(registered_pair_ids), rules = VALUES(rules), court_ids = VALUES(court_ids);
INSERT INTO tournaments (id, name, logo, description, category, level, location, start_date, end_date, status, format, max_pairs, registered_pair_ids, rules, court_ids) VALUES ('tour_nocturno_sevilla', 'Torneo Nocturno Sevilla', '🌙', 'Edición nocturna exprés en formato Eliminación Directa.', 'Masculino', 'Principiante', 'Padel Club Sevilla', '2026-09-20', '2026-09-22', 'UPCOMING', 'Eliminación directa', '8', '["pair_coello_tapia"]', '{"sets_to_win":2,"golden_point":false,"tie_break_at":6,"final_set_tie_break":true,"points_distribution":{"champion":250,"runner_up":150,"semi_finals":90,"quarter_finals":45,"group_stage":20}}', '["crt_2"]') ON DUPLICATE KEY UPDATE id = VALUES(id), name = VALUES(name), logo = VALUES(logo), description = VALUES(description), category = VALUES(category), level = VALUES(level), location = VALUES(location), start_date = VALUES(start_date), end_date = VALUES(end_date), status = VALUES(status), format = VALUES(format), max_pairs = VALUES(max_pairs), registered_pair_ids = VALUES(registered_pair_ids), rules = VALUES(rules), court_ids = VALUES(court_ids);
